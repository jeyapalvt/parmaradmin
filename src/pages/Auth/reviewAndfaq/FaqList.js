...
const FaqList = () => {
  ...
  const getList = async () => {
    await axios
      .post(requests.getAttractionFaqsList, {
        attractionId: attId,
      })
      .then((res) => {
        setrowData(res.data);
  ...
  const columnDefs = [
    { headerName: "Question", field: "faqQuestion" },
    { headerName: "Answer", field: "faqAnswer" },
  ...
  const actionButton = (params) => {
    Swal.fire({
      title: "Warning",
      text: "Are you sure delete this FAQ?",
      icon: "question",
      showDenyButton: true,
      ...
      if (result.isConfirmed) {
        axios
          .post(requests.deleteAttractionFaqs, {
            attractionFaqsId: params.data.attractionFaqsId,
          })
          .then((response) => {
            getList();
            Swal.fire({
              title: "Success",
              text: "FAQ was deleted",
              icon: "success",
            });
          })
          .catch((error) => {
  ...
  return (
    <div>
      <Titlebanner title="FAQ List" />
      <div className="float-right">
        <Link to="/randf/createfaq/null" class="btn btn-primary" role="button">
          Create New
        </Link>
      </div>
      <br />
