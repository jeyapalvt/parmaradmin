...
import Titlebanner from "../title_banner";
import { Col, Row, Button } from "reactstrap";
import * as FileSaver from "file-saver";
import * as XLSX from "xlsx";
import { useHistory } from "react-router-dom";

const ExpirtTktDetails = () => {
  ...
  const getTicketList = (id) => {
    axios
      .post(requests.getTicketsExpiringShortly, { tktBatchNumber: id })
      .then((response) => {
      ...
      if (result.isConfirmed) {
        ...
        axios
          ...
          .then((response) => {
            ...
            const xlName =
              response.data.attractionName + "-" + response.data.ticketTypeName;
  ...
  return (
    <>
      <Titlebanner title="Expiry Tickets Details" />
...
export default ExpirtTktDetails;
